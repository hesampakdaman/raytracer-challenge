...
pub const Gradient = struct {
    ...
    pub fn patternAt(self: *const Gradient, point: Point) Color {
...
pub const Stripe = struct {
    ...
    pub fn patternAt(self: *const Stripe, point: Point) Color {
        const x = @as(i64, @intFromFloat(@floor(point.x())));
...
const TestPattern = if (builtin.is_test) struct {
    ...
    pub fn patternAt(_: *const TestPattern, point: Point) Color {
